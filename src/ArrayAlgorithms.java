public class ArrayAlgorithms
{
    ...
    public static int[] arrayMaximums(int[] intArr1, int[] intArr2) {
        int[] biggest = new int[intArr1.length];
        for (int i = 0; i < biggest.length; i++) {
            biggest[i] = Math.max(intArr1[i], intArr2[i]);
        }
    ...
    public static int howManyExclamations(String[] stringList) {
        ...
        for (String str : stringList) {
            int len = str.length();
            String letter = str.charAt(len - 1) + "";
    ...
    public static int[] shiftLeft(int[] numList) {
        int[] modList = new int[numList.length];

        for (int i = 1; i < numList.length; i++) {
            modList[i - 1] = numList[i];
        }
        modList[numList.length - 1] = numList[0];
    ...
    public static void shiftNumLeftModify2(int[] numList, int shiftNum) {
        ...
        int space = arrAsStr.indexOf(" ");
        for (int i = 0; i < shiftNum; i++) {
            arrAsStr = arrAsStr.substring(space + 1) + arrAsStr.substring(0, space + 1);
            space = arrAsStr.indexOf(" ");
        }
        for (int i = 0; i < numList.length; i++) {
            numList[i] = Integer.parseInt(arrAsStr.substring(0, space));
            arrAsStr = arrAsStr.substring(space + 1);
            space = arrAsStr.indexOf(" ");
        }
    }
    ...
    public static void shiftNumRightModify(int[] numList, int shiftNum) {
        for (int i = 1; i <= numList.length - (shiftNum % numList.length); i++) {
            shiftLeftModify(numList);
        }
    }
    ...
    public static void reverseModify2(int[] numList) {
        for (int i = 0; i < numList.length / 2; i++) {
            int newIdx = numList.length - 1 - i;
            int replacedNum = numList[newIdx];

            numList[newIdx] = numList[i];
            numList[i] = replacedNum;
        }
    }
}